...
try:
    ...
    from splatad import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "splatad_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "splatad_cuda.lib")
        ):
            ...
            with Console().status(
                f"[bold yellow]splatad: Setting up CUDA with MAX_JOBS={os.environ['MAX_JOBS']} (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]splatad: No CUDA toolkit found. splatad will be disabled.[/yellow]"
        )
